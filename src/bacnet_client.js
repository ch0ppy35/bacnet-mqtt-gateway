...
class BacnetClient extends EventEmitter {
  ...
  _readObjectList(deviceAddress, deviceId, callback) {
    ...
    const requestArray = [
      {
        objectId: {
          type: bacnet.enum.ObjectTypes.OBJECT_DEVICE,
          instance: deviceId,
        },
        properties: [{ id: bacnet.enum.PropertyIds.PROP_OBJECT_LIST }],
      },
    ];
  ...
  _mapToDeviceObject(object) {
    ...
    const deviceObjectId = new DeviceObjectId(
      objectInfo.type,
      objectInfo.instance
    );

    const objectProperties = object.values[0].values;
    const name = this._findValueById(
      objectProperties,
      bacnet.enum.PropertyIds.PROP_OBJECT_NAME
    );
    const description = this._findValueById(
      objectProperties,
      bacnet.enum.PropertyIds.PROP_DESCRIPTION
    );
    const type = this._findValueById(
      objectProperties,
      bacnet.enum.PropertyIds.PROP_OBJECT_TYPE
    );
    const units = this._findValueById(
      objectProperties,
      bacnet.enum.PropertyIds.PROP_UNITS
    );
    const presentValue = this._findValueById(
      objectProperties,
  ...
  scanDevice(device) {
    return new Promise((resolve, reject) => {
      logger.log(
        "info",
        `Reading full object list from device: ${device.address}`
      );
      this._readObjectList(device.address, device.deviceId, (err, result) => {
        if (!err) {
          ...
          Promise.all(promises)
            .then((result) => {
              const successfulResults = result.filter(
                (element) => !element.error
              );
              const deviceObjects = successfulResults.map((element) =>
                this._mapToDeviceObject(element.value)
              );
  ...
  startPolling(device, objects, scheduleExpression) {
    logger.log(
      "info",
      `Schedule polling for device ${device.address} with expression ${scheduleExpression}`
    );
    scheduleJob(scheduleExpression, () => {
      ...
      objects.forEach((deviceObject) => {
        promises.push(
          this._readObjectPresentValue(
            device.address,
            deviceObject.objectId.type,
            deviceObject.objectId.instance
          )
        );
      });
      Promise.all(promises)
        .then((result) => {
          ...
          successfulResults.forEach((object) => {
            const objectId =
              object.values[0].objectId.type +
              "_" +
              object.values[0].objectId.instance;
            const presentValue = this._findValueById(
              object.values[0].values,
              bacnet.enum.PropertyIds.PROP_PRESENT_VALUE
            );
            const objectName = this._findValueById(
              object.values[0].values,
              bacnet.enum.PropertyIds.PROP_OBJECT_NAME
            );
